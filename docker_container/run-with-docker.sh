...
" >&2; exit 127; 
  ...
  $DRY_RUN "${docker_cmd}" run --name gp_job_$JOB_NO \
    --workdir "${workdir}" \
    --mount "src=genepattern_taskLib,dst=/opt/gp/taskLib" --mount "src=genepattern_jobResults,dst=/opt/gp/jobResults" --mount "src=genepattern_users,dst=/opt/gp/users" \
    "${docker_img}" \
    "${__gp_module_cmd[@]}";

  # job is done, remove the container
